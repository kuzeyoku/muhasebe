<div class="modal-dialog modal-xl">
    <div class="modal-content">
        ...
        {{html()->form("PUT")->route("user.update",$user)->open()}}
        <div class="modal-body">
            <div class="mb-2">
                {{html()->label("Adı Soyadı", "name")->class("form-label")->for("name")}}
                {{html()->text("name",$user->name)->class("form-control")->placeholder("Adı Soyadı")->required()}}
            </div>
            <div class="mb-2">
                {{html()->label("Email Adresi", "email")->class("form-label")->for("email")}}
                {{html()->email("email",$user->email)->class("form-control")->placeholder("Email Adresi")->required()}}
            </div>
            <div class="mb-2">
                {{html()->label("Şifre", "password")->class("form-label")->for("password")}}
                {{html()->password("password")->class("form-control")->placeholder("Şifre")}}
            </div>
            <div class="mb-2">
                {{html()->label("Şifre Tekrar", "password_confirmation")->class("form-label")->for("password_confirmation")}}
                {{html()->password("password_confirmation")->class("form-control")->placeholder("Şifre Tekrar")}}
            </div>
