<div class="modal-dialog modal-xl">
    <div class="modal-content">
        ...
        {{html()->form()->route("debit.store")->open()}}
        <div class="modal-body">
            <div class="mb-2">
                {{html()->label("Alacaklı Ünvanı", "name")->class("form-label")->for("name")}}
                {{html()->text("name")->class("form-control")->placeholder("Alacaklı Ünvanı")->required()}}
            </div>
            <div class="mb-2">
                {{html()->label("Borç Tutarı", "amount")->class("form-label")->for("amount")}}
                {{html()->number("amount")->class("form-control")->placeholder("Borç Tutarı")->required()}}
            </div>
            <div class="mb-2">
                {{html()->label("Borç Vadesi", "end_date")->class("form-label")->for("end_date")}}
                {{html()->date("due_date")->class("form-control")->placeholder("Borç Vadesi")->required()}}
            </div>
            <div class="mb-2">
