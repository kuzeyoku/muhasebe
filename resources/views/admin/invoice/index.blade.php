...
@section("button")
    <button id="create_button" data-url="{{route("admin.invoice.create")}}" class="btn btn-sm btn-blue">Fatura Ekle
    </button>
@endsection
@section("content")
    <table class="table table-striped border">
        ...
        @foreach($invoices as $invoice)
            <tr class="bg-{{$invoice->type == "income" ? "soft-success":"soft-danger"}}">
                ...
                <td>
                    <strong class="text-{{$invoice->status == "paid" ? "success" : "danger"}}">{{$invoice->statusName}}</strong>
                </td>
                <td class=" text-end">
                    <button type="button" data-url="{{route("admin.invoice.edit", $invoice)}}"
                            class="btn btn-sm btn-blue edit_button"><i
                                class="las la-pen"></i></button>
                    {{html()->form("DELETE", route("admin.invoice.destroy", $invoice))->class("d-inline")->open()}}
